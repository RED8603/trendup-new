...
import { useGetTrendingFeedQuery } from '@/api/slices/socialApi';
...
const TrendingFeed = () => {
  ...
  const { data, error, isLoading, refetch } = useGetTrendingFeedQuery(
    { page: 1, limit: 20 },
    { skip: isGuestMode }
  );
  ...
  if (isGuestMode || (error && (error.data?.message?.includes('token') || error.data?.message?.includes('Token') || error.data?.message?.includes('No token')))) {
    return <GuestFeedError featureName="the Trending feed" />;
  }
