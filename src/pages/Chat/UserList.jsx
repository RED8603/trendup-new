...
export default function UserList({
  ...
  return (
    <BoxConatner rsx={{ height: "100%",  display: "flex", flexDirection: "column", }}>
      {/* Server Header */}
              ...
              <ListItem key={user.id} disablePadding>
                ...
                >
                  <ListItemAvatar>
                    ...
                    >
                      <Avatar src={user.avatar} sx={{ bgcolor: user.color, width: 40, height: 40 }}>
                        {user.name[0]}
                      </Avatar>
                    </Badge>
                  </ListItemAvatar>
                  <ListItemText
                    ...
                    secondary={
                      user.status === "offline" && user.lastSeen ? (
                        ...
                        <Typography
                          variant="caption"
                          sx={{ color: getStatusColor(user.status), textTransform: "capitalize" }}
                        >
                          {user.status}
                        </Typography>
                      )
